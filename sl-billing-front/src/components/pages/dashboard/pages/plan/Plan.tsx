...
import { fetchPlans } from "../../../../../store/planSlice/action";
import { useSelector } from "react-redux";
import Loading from "../../../../parts/load/loading";
import PlanRowTable from "./parts/RowTable/PlanRowTable";
import { PlanType } from "../../../../../@Types/PlanType";
import PlanModalAddDelete from "./parts/planModalAddDelete/PlanModalAddDelete";
import DeleteModal from "./parts/deletePlan/DeleteModal";
...
const Plan = () => {
  ...
  const [planState, setPlanState] = useState<PlanType>({ planName: "" });
  const [deleteModal, setDeleteModal] = useState<boolean>(false);
  const [planDelete, setPlanDelete] = useState<PlanType>({ planName: "" });

  const showModalDelete = (payload: PlanType) => {
    setDeleteModal(true);
    setPlanDelete(payload);
  };

  const handleShowModal = (payload: string, plan: PlanType) => {
    setShowModal(true);
    setAction(payload);
    if (payload === "update") {
      setPlanState(plan);
  ...
  const handleCloseModal = () => {
    ...
    setPlanDelete({ planName: "" });
    setPlanState({ planName: "" });
  };
  ...
  const dispatchDebounce = debounce(() => {
    dispatch(fetchPlans({ limit: 5, planName: search, skip: (page - 1) * 5 }));
  }, 0);
  ...
  const [teamsPlan, setTeamsPlan] = useState<TeamType[]>([]);

  const getAllTeamsPlan = debounce(() => {
    getAllTeam(setTeamsPlan);
  }, 0);
      ...
      <div className="mt-5 flex justify-center">
        <nav aria-label="Page navigation example">
          <ul className="inline-flex -space-x-px">
            ...
            {[...Array(Math.ceil(plan.count / 5))].map((el, idx) => {
              return (
                <li key={idx + 1}>
                  <button
                    disabled={page === idx + 1}
            ...
            <li>
              <button
                disabled={page === Math.ceil(plan.count / 5)}
                onClick={() => {
                  setPage(page + 1);
                }}
                className={
                  page === Math.ceil(plan.count / 5)
