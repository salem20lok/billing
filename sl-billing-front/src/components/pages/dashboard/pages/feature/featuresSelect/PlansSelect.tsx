...
import { getPlansPagination } from "../../../../../../actions/plan/action";
import { PlanType } from "../../../../../../@Types/PlanType";

interface PlansSelectPropsInterface {
  handlePlans: Function;
  handleDeleteAllPlans: Function;
}

const PlansSelect = (props: PlansSelectPropsInterface) => {
  const { handlePlans, handleDeleteAllPlans } = props;
  ...
  const planPaginationDebounce = debounce((query) => {
    getPlansPagination(query, (data) => {
      if (data.count <= plans.length + 8) setHasMore(false);
      setPlans([...plans, ...data.plans]);
    });
  }, 0);

  const handlePlansPagination = () => {
    setQueryFeature({
      ...queryFeature,
  ...
  useEffect(() => {
    planPaginationDebounce(queryFeature);
