...
import { fetchFeatures } from "../../../../../store/featureSlice/action";
import RowTableFeature from "./RowTable/RowTableFeature";
import FeatureModel from "./ModlaAddDeleteFeature/FeatureModel";
...
const Feature = () => {
  ...
  const [featureState, setFeatureState] = useState<FeatureType>({
    featureName: "",
  });
  const [deleteModal, setDeleteModal] = useState<boolean>(false);
  const [featureDelete, setFeatureDelete] = useState<FeatureType>({
    featureName: "",
  });

  const showModalDelete = (payload: FeatureType) => {
    setDeleteModal(true);
    setFeatureDelete(payload);
  };

  const handleShowModal = (payload: string, feature: FeatureType) => {
    setShowModal(true);
    setAction(payload);
    if (payload === "update") {
  ...
  const handleCloseModal = () => {
    ...
    setFeatureDelete({ featureName: "" });
    setFeatureState({ featureName: "" });
  };
  ...
  const dispatchDebounce = debounce(() => {
    dispatch(
      fetchFeatures({ limit: 5, featureName: search, skip: (page - 1) * 5 })
    );
  }, 0);
      ...
      <div className=" flex justify-between items-center pb-4  ">
        ...
        {(consumer.permission.some((el) => {
          return (
            ...
            el.permission.management === PermissionEnum.Feature
          );
        }) ||
          consumer.user.role === RoleEnumFront.SuperAdmin) && (
          <button
            onClick={() => {
              handleShowModal("add", featureState);
            }}
            type="button"
      ...
      <div className="mt-5 flex justify-center">
        <nav aria-label="Page navigation example">
          <ul className="inline-flex -space-x-px">
            ...
            {[...Array(Math.ceil(features.count / 5))].map((el, idx) => {
              return (
                <li key={idx + 1}>
                  <button
                    disabled={page === idx + 1}
            ...
            <li>
              <button
                disabled={page === Math.ceil(features.count / 5)}
                onClick={() => {
                  setPage(page + 1);
                }}
                className={
                  page === Math.ceil(features.count / 5)
